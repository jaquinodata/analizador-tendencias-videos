"""
Modelo Random Forest Regressor de Sklearn

Clase: RandomForestReg
"""
...
from models.preprocess_strategies import AplicarStandardScaler
...
class RandomForestReg:
    ...
    save: Guarda en disco serializado en pickle el objeto RandomForestReg actual
    load: Carga de disco un objeto RandomForestReg con el entrenamiento del modelo
    ...
    def fit(self, df, variable):
        ...
        # max_samples: None, int, float (0.0, 1.0]
        max_samples = config.get(SECCION_CONFIG + variable, 'max_samples')
        if max_samples == 'None':
            max_samples = None
        else:
            try:
        ...
        # oob_score: bool
        oob_score = config.getboolean(SECCION_CONFIG + variable, 'oob_score')

        # random_state: int
        random_state = config.getint(SECCION_CONFIG + variable, 'random_state')

        params_rf = {'bootstrap': bootstrap, 'ccp_alpha': ccp_alpha, 'criterion': criterion,
                     'max_depth': max_depth, 'max_features': max_features, 'max_samples': max_samples,
                     'max_leaf_nodes': max_leaf_nodes, 'min_impurity_decrease': min_impurity_decrease,
                     'min_samples_leaf': min_samples_leaf, 'min_samples_split': min_samples_split,
                     'n_estimators': n_estimators, 'oob_score': oob_score, 'random_state': random_state}

        # instancia Random Forest
        self.model = RandomForestRegressor(**params_rf)
    ...
    def save(self, directory_save, variable):
        '''
        Guarda en disco serializado en pickle el objeto RandomForestReg actual
    ...
    def load(self, directory_load, variable):
        '''
        Carga de disco un objeto RandomForestReg con el entrenamiento del modelo
    ...
    def __preprocess_data(self, df, variable):
        ...
        match variable:
            case 'ratio_likes_dislikes':

                # elimina registros con dislikes a cero
                df_ratio = df[df['dislikes'].apply(lambda x: x != 0)].copy()

                # crea la nueva variable
                df_ratio['ratio_likes_dislikes'] = df_ratio.apply(lambda row: row['likes'] / row['dislikes'], axis=1)

                # escala la nueva variable
                preprocessing_strategy = AplicarStandardScaler()
                data = df_ratio['ratio_likes_dislikes'].to_numpy().reshape(-1, 1)
                model_ratio, df_ratio['ratio_likes_dislikes_encoded'] = preprocessing_strategy.preprocess(data)

                # selecciona las variables de entrada de entre las columnas del dataset
                X = df_ratio[['channel_title_encoded', 'tags_encoded', 'views_encoded', 'comment_count_encoded',
                ...
                y = df_ratio[['ratio_likes_dislikes_encoded']].copy(deep=True)
